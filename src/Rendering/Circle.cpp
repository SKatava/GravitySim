...
Circle::Circle() : m_VAO(), m_VBO(), m_radius(0.05), m_pos(0.f, 0.f), m_color(1.f, 1.f, 1.f){
    m_VAO.Bind();
    m_VBO.Bind();
...
void Circle::Update(){
    ...
    for (int i = 0; i < 36; ++i) {
        ...
        vertices[i * 6] = m_pos.x + m_radius * std::cos(angle);
        vertices[i * 6 + 1] = m_pos.y + m_radius * std::sin(angle);
        vertices[i * 6 + 2] = 0.f;

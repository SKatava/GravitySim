...
void GUI::InitGUI(GLFWwindow* window){
    ...
    ImFont* iconFont = io.Fonts->AddFontFromFileTTF("../assets/fonts/fontawesome-desktop/otfs/Font-Awesome-Free-Solid-900.otf", 16.0f, &config, icons_ranges);
    io.Fonts->Build();
    ...
    plotStyle.Colors[ImPlotCol_AxisBgActive] = ImVec4(0, 0, 0, 0);
...
void GUI::TimeInfo(){
    ...
    ImGui::TableSetColumnIndex(1); 
    ImGui::Text("%s", Time::FormatTime(Time::TotalRealTime).c_str());
    
    ImGui::TableNextRow();
    ImGui::TableSetColumnIndex(0);
    ImGui::Text("Total simulation time");
    ImGui::TableSetColumnIndex(1);
    ImGui::Text("%s", Time::FormatTime(Time::TotalSimTime).c_str());
    
    ImGui::TableNextRow();
    ImGui::TableSetColumnIndex(0);
    ImGui::Text("FPS");
    ImGui::TableSetColumnIndex(1);
    ImGui::Text("%d", Time::FPS);
    ...
    if(Settings::time_mode == TimeMode::REAL_TIME){
        m_NewDeltaTime = Time::deltaTime;
        ImGui::BeginDisabled();
        ImGui::InputFloat("INPUT_DELTA_TIME", &m_NewDeltaTime, 0, 0, "0.2f");
        ImGui::EndDisabled();
    }
    else{
        if(ImGui::InputFloat("##INPUT_DELTA_TIME", &m_NewDeltaTime, 0, 0, "%.4f")){
            Time::deltaTime = m_NewDeltaTime;   
        }
    }
...
void GUI::AccelerationOptions(){
    ...
    const char* categories[] = { "Low", "Medium", "High", "Ultra" };
    ...
    if(ImGui::SliderInt("##quality_slider", &current, 0, IM_ARRAYSIZE(categories) - 1, "")){
       ...
       else if(current == 2){Settings::mode = Mode::GPU_ACCELERATION;}
       else if(current == 3){Settings::mode = Mode::ULTRA;}
    }
    ImGui::End();
}
...
void GUI::UsageAnalytics(){
    ...
    ImPlot::SetNextAxesLimits(0, 10, 0, 100, ImPlotCond_Always);
...
void GUI::TimePlot(){
    ...
    values[0] = Time::PhysicsEngineTime;
    values[1] = Time::IntegratorTime;
    values[2] = Time::EventProcessingTime;
    values[3] = Time::RenderingTime;
        ...
        if(ImGui::Button("Confirm", ImVec2(125, 40))){
            if(m_NumToAdd == 1){
                if(m_EntityTypeSelected == 0){
                    Entities::CreateEntity(m_NewMass, m_NewSize, Vector2d(m_NewPosX, m_NewPosY), EntityType::OBJECT, Vector3f(m_NewColor[0], m_NewColor[1], m_NewColor[2]), m_newName);
                }
                else if(m_EntityTypeSelected == 1){
                    Entities::CreateEntity(m_NewMass, m_NewSize, Vector2d(m_NewPosX, m_NewPosY), EntityType::PARTICLE, Vector3f(m_NewColor[0], m_NewColor[1], m_NewColor[2]), m_newName);
                }
            }
            else{
                if(m_EntityTypeSelected == 0){
                    Entities::CreateBachOfEntities(m_NewMass, m_NewSize, Vector2d(m_NewPosX, m_NewPosY), EntityType::OBJECT, Vector3f(m_NewColor[0], m_NewColor[1], m_NewColor[2]), m_newName, m_NumToAdd);
                }
                else if(m_EntityTypeSelected == 1){
                    Entities::CreateBachOfEntities(m_NewMass, m_NewSize, Vector2d(m_NewPosX, m_NewPosY), EntityType::PARTICLE, Vector3f(m_NewColor[0], m_NewColor[1], m_NewColor[2]), m_newName, m_NumToAdd);
                }
            }
